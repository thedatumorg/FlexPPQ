...
/* FPGrowth */
inline SetPattern findFrequentPattern(const CodebookType &codebook, uint64_t minimum_support_threshold, const std::vector<int> &centroidsNum) {
...
inline std::vector<int> parseVAQHardcode(std::string str) {
  std::vector<int> ret;
  ret.reserve(64);
...
// double getAvgRecall(const std::vector<int>& groundTruthLabels, 
//                     const std::vector<std::vector<int>>& predictedTopNN, 
//                     const int numNeighbors) {
//   double averageRecall = 0.0;

//   int numQueries = groundTruthLabels.size() / numNeighbors;
//   for (int queryIndex = 0; queryIndex < numQueries; queryIndex++) {
//       int matchCount = 0;
//       for (int neighborIndex = 0; neighborIndex < numNeighbors; neighborIndex++) {
//           for (int topIndex = 0; topIndex < numNeighbors; topIndex++) {
//               if (groundTruthLabels[queryIndex * numNeighbors + neighborIndex] == 
//                   predictedTopNN[queryIndex][topIndex] - IdxOffset) {
//                   matchCount++;
//                   break;
//               }
//           }
//       }
//       averageRecall += static_cast<double>(matchCount) / numNeighbors;
//     }
//   averageRecall /= numQueries;
//   return averageRecall;
// }

/*
The performance measure is recall@R, that is, for varying values of R, 
the average rate of queries for which the 1-nearest neighbor(ground truth) is ranked in the top R positions(prediction).
Please use this measure to allow a direct comparison of your system with most 
of the results reported in the literature. 
Ref: http://corpus-texmex.irisa.fr/ https://www.cnblogs.com/thmyl/p/18065408
*/
// template <int IndexOffset = 0>
// double getRecallAtR(const std::vector<int>& predictedTopNN, 
//                              const std::vector<std::vector<int>>& groundTruthLabels, 
//                              const int numNeighbors, const int R) {
//     int matchCount = 0;

//     int numQueries = predictedTopNN.size() / numNeighbors;
//     for (int queryIndex = 0; queryIndex < numQueries; queryIndex++) {

//         int truenn = groundTruthLabels[queryIndex][0];
//         for (int topIndex = 0; topIndex < R; topIndex++) {
//             if (predictedTopNN[queryIndex * numNeighbors + topIndex] - IndexOffset == truenn) {
//                 matchCount++;
//                 break;
//             }
//         }
...
//     // 计算平均 Recall
//     double averageRecall = static_cast<double>(matchCount) / numQueries;
//     return averageRecall;
// }
...
double getRecallAtR(const std::vector<int>& predictedTopNN, 
                             const std::vector<std::vector<int>>& groundTruthLabels, 
                             const int numNeighbors, const int R, bool RAtR = false) {
    ...
    if (!RAtR){
      ...
      for (int queryIndex = 0; queryIndex < numQueries; queryIndex++) {
          ...
          for (int topIndex = 0; topIndex < R; topIndex++) {
            if (hit) break;
            for(int gtIndex = 0; gtIndex < R; ++gtIndex) {
              if (hit) break;
              int truenn = groundTruthLabels[queryIndex][gtIndex];
              if (predictedTopNN[queryIndex * numNeighbors + topIndex] - IndexOffset == truenn) {
                  matchCount++;
                  hit = true;
              }
      ...
      for (int queryIndex = 0; queryIndex < numQueries; queryIndex++) {
          int hitCount = 0;  // count how many true neighbors are found

          for (int topIndex = 0; topIndex < R; topIndex++) {
              int pred = predictedTopNN[queryIndex * numNeighbors + topIndex] - IndexOffset;

              for (int gtIndex = 0; gtIndex < R; gtIndex++) {
                  int truth = groundTruthLabels[queryIndex][gtIndex];
                  if (pred == truth) {
                      hitCount++;
                      break;     // this pred cannot match another GT
                  }
              }
          }
      ...
      // final recall R@R
      double recallR_at_R = static_cast<double>(totalHits) / (numQueries * R);
      return recallR_at_R;
    }
}

template <int IdxOffset=0>
double getAvgRecall(const std::vector<int>& predictedTopNN, 
                    const std::vector<std::vector<int>>& groundTruthLabels, 
                    const int numNeighbors, bool RAtR = false) {
  return getRecallAtR<IdxOffset>(predictedTopNN, groundTruthLabels, numNeighbors, numNeighbors, RAtR);
}

// From ann-benchmark, and used by DeltaPQ
// Ref codes: https://github.com/erikbern/ann-benchmarks/blob/266b14f6a4e630b21f24e703c5dba721a7b27316/ann_benchmarks/plotting/metrics.py#L14
// ann-benchmark: https://github.com/erikbern/ann-benchmarks
inline double getEpsilonRecallAtR(const std::vector<float>& predictedTopNNDistances, 
                             const std::vector<std::vector<int>>& groundTruthLabels, 
                             const RowMatrixXf& queries, const RowMatrixXf&dataset,
                             const int numNeighbors, const int R, const double epsilon=1e-1) {
    int matchCount = 0;

    int numQueries = predictedTopNNDistances.size() / numNeighbors;
    for (int queryIndex = 0; queryIndex < numQueries; queryIndex++) {

        int truenn = groundTruthLabels[queryIndex][numNeighbors - 1];
        float epsilonDistance = fvec_L2sqr(queries.row(queryIndex).data(), dataset.row(truenn).data(), dataset.cols());
        epsilonDistance *= (1+epsilon);
        // need distance b/t turenn and query
        for (int topIndex = 0; topIndex < R; topIndex++) {
            if (predictedTopNNDistances[queryIndex * numNeighbors + topIndex] <= epsilonDistance) {
                matchCount++;
                break;
            }
        }
...
// Recall from VAQ
// If the 1st nearest groudtruth appear at topk prediction, then match
// template <int IdxOffset=0>
// double getRecall(const std::vector<int> &labels, const std::vector<std::vector<int>> &topnn, const int K) {
...
*/
// template <int IdxOffset=0>
// double getRecallAtR(const std::vector<int> &labels, const std::vector<std::vector<int>> &topnn, const int K, const int R) {
//     int nq = labels.size() / K;
//     double ans = 0.0;
...
//         for (int k_idx = 0; k_idx < std::min(R, K); k_idx++) {
//             if (truenn - IdxOffset == labels[p_idx * K + k_idx]) {
//                 ans += 1;
//                 break;
//             }
...
// }
template <int IdxOffset=0>
double getMeanAveragePrecisionOld(const std::vector<int>& labels, const std::vector<std::vector<int>> &topnn, const int K) {
  double ans = 0.0;
...
// A good note to explain this: https://www.educative.io/answers/what-is-the-mean-average-precision-in-information-retrieval
// Ref: https://dl-acm-org.proxy.lib.ohio-state.edu/doi/epdf/10.14778/3204028.3204034 (Hd-Index)
// Please check Example I in Ref to understand the codes
// Also used by DeltaPQ
template <int IdxOffset = 0>
...
template <int IdxOffset=0>
double getMeanAveragePrecision(const std::vector<int>& predictedTopNN, 
                              const std::vector<std::vector<int>>& groundTruthLabels, 
                              const int numNeighbors) {
    return getMeanAveragePrecisionAtR<IdxOffset>(predictedTopNN, groundTruthLabels, numNeighbors, numNeighbors);
}

/* Clustering functions for BitVecEngine */
inline void computeClusterIndex(const Matrixf &dataset, const Matrixf& centroids, std::vector<int> &clusterIdx) {
...
inline void computeClusterIndexDiskResident(std::string filepath, const Matrixf& centroids, std::vector<int> &clusterIdx, int N, int batch=10000000) {
